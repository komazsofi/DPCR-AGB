...
class Wandb:
    ...
    def launch(cfg, launch: bool):
        if launch:
            ...
            optim = cfg.training.get("optim", None)
            otimizer_class = getattr(optim.optimizer, "class") if optim is not None else "loaded model"
            lr_scheduler = cfg.training.get("lr_scheduler", None)
            scheduler_class = getattr(lr_scheduler, "class") if lr_scheduler is not None else "loaded model"
            tags = [
                cfg.model_name,
                model_class.split(".")[0],
            ...
            wandb_args["settings"] = wandb.Settings(start_method="fork")
